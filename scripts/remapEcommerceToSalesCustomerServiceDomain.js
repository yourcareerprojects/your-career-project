#!/usr/bin/env node
/**
 * Remap legacy "E-commerce" / "E-Commerce" domain data to "Sales and Customer Service".
 *
 * Updates:
 * - CareerPath.domain
 ...
 *
 * Usage:
 *   node scripts/remapEcommerceToSalesCustomerServiceDomain.js
 *   node scripts/remapEcommerceToSalesCustomerServiceDomain.js --dry-run
 *   node scripts/remapEcommerceToSalesCustomerServiceDomain.js --limit=100
 */
...
const TARGET_DOMAIN = 'Sales and Customer Service';
const LEGACY_DOMAIN_VALUES = ['E-commerce', 'E-Commerce', 'Ecommerce', 'e-commerce'];
...
function isLegacyEcommerceValue(value) {
...
function remapDomainArray(values) {
  ...
  const hadLegacy = values.some(isLegacyEcommerceValue);
...
async function migrateCareerPaths({ dryRun, limit }) {
  ...
  console.log(
    `[remapEcommerceToSalesCustomerServiceDomain] CareerPath matches: ${matching}${dryRun ? ' (dry-run)' : ''}`
  );
...
async function migrateUsers({ dryRun, limit }) {
  ...
  console.log(
    `[remapEcommerceToSalesCustomerServiceDomain] User matches: ${matching}${dryRun ? ' (dry-run)' : ''}`
  );
  if (matching === 0) {
    return { matched: 0, modified: 0 };
  }
...
async function main() {
  ...
  console.log(
    `[remapEcommerceToSalesCustomerServiceDomain] CareerPath updated: ${careerPathResult.modified} / ${careerPathResult.matched}`
  );
  console.log(
    `[remapEcommerceToSalesCustomerServiceDomain] Users updated: ${userResult.modified} / ${userResult.matched}`
  );
...
main().catch(async (err) => {
  console.error('[remapEcommerceToSalesCustomerServiceDomain] failed:', err);
  try {
    await mongoose.disconnect();
  } catch (_) {
    /* ignore */
