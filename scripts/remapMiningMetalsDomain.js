#!/usr/bin/env node
/**
 * Remap legacy "Mining & Metals" domain data to "Mining".
 *
 * Updates:
 * - CareerPath.domain
 ...
 *
 * Usage:
 *   node scripts/remapMiningMetalsDomain.js
 *   node scripts/remapMiningMetalsDomain.js --dry-run
 *   node scripts/remapMiningMetalsDomain.js --limit=100
 */
...
const TARGET_DOMAIN = 'Mining';
const LEGACY_DOMAIN_VALUES = ['Mining & Metals', 'Bergbau & Metalle'];
const LEGACY_DOMAIN_KEYS = new Set(['mining & metals', 'bergbau & metalle']);
...
function isLegacyMiningMetalsValue(value) {
...
function remapDomainArray(values) {
  ...
  const hadLegacy = values.some(isLegacyMiningMetalsValue);
...
async function migrateCareerPaths({ dryRun, limit }) {
  ...
  console.log(`[remapMiningMetalsDomain] CareerPath matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
...
async function migrateUsers({ dryRun, limit }) {
  ...
  console.log(`[remapMiningMetalsDomain] User matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
  if (matching === 0) {
    return { matched: 0, modified: 0 };
  }
...
async function main() {
  ...
  console.log(
    `[remapMiningMetalsDomain] CareerPath updated: ${careerPathResult.modified} / ${careerPathResult.matched}`
  );
  console.log(
    `[remapMiningMetalsDomain] Users updated: ${userResult.modified} / ${userResult.matched}`
  );
...
main().catch(async (err) => {
  console.error('[remapMiningMetalsDomain] failed:', err);
  try {
    await mongoose.disconnect();
  } catch (_) {
    /* ignore */
