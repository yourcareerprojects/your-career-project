#!/usr/bin/env node
/**
 * Remap legacy "Automotive Trades" domain data to "Automotive".
 *
 * Updates:
 * - CareerPath.domain
 ...
 *
 * Usage:
 *   node scripts/remapAutomotiveTradesDomain.js
 *   node scripts/remapAutomotiveTradesDomain.js --dry-run
 *   node scripts/remapAutomotiveTradesDomain.js --limit=100
 */
...
const TARGET_DOMAIN = 'Automotive';
const LEGACY_DOMAIN_KEYS = new Set(['automotive trades', 'kfz-handwerk', 'kfz handwerk']);
...
function isLegacyAutomotiveTradeValue(value) {
...
function remapDomainArray(values) {
  ...
  const hadLegacy = values.some(isLegacyAutomotiveTradeValue);
...
async function migrateCareerPaths({ dryRun, limit }) {
  const filter = { domain: { $in: ['Automotive Trades', 'Kfz-Handwerk'] } };
  const matching = await CareerPath.countDocuments(filter);
  console.log(`[remapAutomotiveTradesDomain] CareerPath matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
...
async function migrateUsers({ dryRun, limit }) {
  const query = {
    $or: [
      { 'profile.careerPreferences.domains': { $in: ['Automotive Trades', 'Kfz-Handwerk'] } },
      { 'profile.structuredUserInfo.domains.raw_items': { $in: ['Automotive Trades', 'Kfz-Handwerk'] } },
      { 'profile.careerSimulationInputs.structuredUserInfo.domains.raw_items': { $in: ['Automotive Trades', 'Kfz-Handwerk'] } },
    ],
  };

  const matching = await User.countDocuments(query);
  console.log(`[remapAutomotiveTradesDomain] User matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
  if (matching === 0) {
    return { matched: 0, modified: 0 };
  }
...
async function main() {
  ...
  console.log(
    `[remapAutomotiveTradesDomain] CareerPath updated: ${careerPathResult.modified} / ${careerPathResult.matched}`
  );
  console.log(
    `[remapAutomotiveTradesDomain] Users updated: ${userResult.modified} / ${userResult.matched}`
  );
...
main().catch(async (err) => {
  console.error('[remapAutomotiveTradesDomain] failed:', err);
  try {
    await mongoose.disconnect();
  } catch (_) {
    /* ignore */
