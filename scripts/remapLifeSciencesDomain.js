#!/usr/bin/env node
/**
 * Remap legacy "Life Sciences" domain data to "Natural Sciences".
 *
 * Updates:
 * - CareerPath.domain
 ...
 *
 * Usage:
 *   node scripts/remapLifeSciencesDomain.js
 *   node scripts/remapLifeSciencesDomain.js --dry-run
 *   node scripts/remapLifeSciencesDomain.js --limit=100
 */
...
function isLegacyLifeSciencesValue(value) {
...
function remapDomainArray(values) {
  ...
  const hadLegacy = values.some(isLegacyLifeSciencesValue);
...
async function migrateCareerPaths({ dryRun, limit }) {
  ...
  console.log(`[remapLifeSciencesDomain] CareerPath matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
...
async function migrateUsers({ dryRun, limit }) {
  ...
  console.log(`[remapLifeSciencesDomain] User matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
  if (matching === 0) {
    return { matched: 0, modified: 0 };
  }
...
async function main() {
  ...
  console.log(
    `[remapLifeSciencesDomain] CareerPath updated: ${careerPathResult.modified} / ${careerPathResult.matched}`
  );
  console.log(
    `[remapLifeSciencesDomain] Users updated: ${userResult.modified} / ${userResult.matched}`
  );
...
main().catch(async (err) => {
  console.error('[remapLifeSciencesDomain] failed:', err);
  try {
    await mongoose.disconnect();
  } catch (_) {
    /* ignore */
