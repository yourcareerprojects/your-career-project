#!/usr/bin/env node
/**
 * Merge all CareerPath roles whose title contains "distribution manager"
 * into one canonical role (title: "distribution manager", escoId of that record).
 *
 * Same merge pipeline as deduplicateRolesByEmbedding (--apply).
 *
 * Usage:
 *   node scripts/mergeDistributionManagerCluster.js --dry-run
 *   node scripts/mergeDistributionManagerCluster.js --apply
 */
...
const CANONICAL_TITLE = 'distribution manager';
...
function pickCanonicalEscoId(docs) {
  const exact = docs.filter((d) => String(d.title || '').trim().toLowerCase() === CANONICAL_TITLE);
...
async function main() {
  ...
  const docs = await CareerPath.find(
    { title: { $regex: 'distribution manager', $options: 'i' } },
    {},
  )
    .sort({ escoId: 1 })
    .lean();
  ...
  if (docs.length === 1) {
    ...
    const already = m.length > 0 && docs[0].title?.toLowerCase() === CANONICAL_TITLE;
    console.log(
      JSON.stringify({
        ...
        likely_already_merged: already && m.length >= 30,
      }),
    );
    await mongoose.disconnect();
    process.exit(0);
  ...
  if (flags.dryRun) {
    console.log(
      JSON.stringify(
        {
          summary: report,
          alternative_titles_sample: merged.altTitles.slice(0, 15),
