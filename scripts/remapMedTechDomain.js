#!/usr/bin/env node
/**
 * Remap legacy "MedTech" domain data to "Healthcare".
 *
 * Updates:
 * - CareerPath.domain
 ...
 *
 * Usage:
 *   node scripts/remapMedTechDomain.js
 *   node scripts/remapMedTechDomain.js --dry-run
 *   node scripts/remapMedTechDomain.js --limit=100
 */
...
const TARGET_DOMAIN = 'Healthcare';
...
function isLegacyMedTechValue(value) {
...
function remapDomainArray(values) {
  ...
  const hadLegacy = values.some(isLegacyMedTechValue);
...
async function migrateCareerPaths({ dryRun, limit }) {
  ...
  console.log(`[remapMedTechDomain] CareerPath matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
...
async function migrateUsers({ dryRun, limit }) {
  ...
  console.log(`[remapMedTechDomain] User matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
  if (matching === 0) {
    return { matched: 0, modified: 0 };
  }
...
async function main() {
  ...
  console.log(
    `[remapMedTechDomain] CareerPath updated: ${careerPathResult.modified} / ${careerPathResult.matched}`
  );
  console.log(
    `[remapMedTechDomain] Users updated: ${userResult.modified} / ${userResult.matched}`
  );
...
main().catch(async (err) => {
  console.error('[remapMedTechDomain] failed:', err);
  try {
    await mongoose.disconnect();
  } catch (_) {
    /* ignore */
