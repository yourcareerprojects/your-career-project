#!/usr/bin/env node
/**
 * Remap legacy natural-science domains to "Natural Sciences".
 *
 * Merges:
 * - Biology / Biologie
 * - Chemistry / Chemiewissenschaften
 * - Chemicals / Chemie
 * - Physics / Physik
 ...
 *
 * Usage:
 *   node scripts/remapNaturalSciencesDomain.js
 *   node scripts/remapNaturalSciencesDomain.js --dry-run
 *   node scripts/remapNaturalSciencesDomain.js --limit=100
 */
...
const TARGET_DOMAIN = 'Natural Sciences';
...
function isLegacyNaturalSciencesValue(value) {
...
function remapDomainArray(values) {
  ...
  const hadLegacy = values.some(isLegacyNaturalSciencesValue);
...
async function migrateCareerPaths({ dryRun, limit }) {
  ...
  console.log(`[remapNaturalSciencesDomain] CareerPath matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
...
async function migrateUsers({ dryRun, limit }) {
  ...
  console.log(`[remapNaturalSciencesDomain] User matches: ${matching}${dryRun ? ' (dry-run)' : ''}`);
  if (matching === 0) {
    return { matched: 0, modified: 0 };
  }
...
async function main() {
  ...
  console.log(
    `[remapNaturalSciencesDomain] CareerPath updated: ${careerPathResult.modified} / ${careerPathResult.matched}`
  );
  console.log(
    `[remapNaturalSciencesDomain] Users updated: ${userResult.modified} / ${userResult.matched}`
  );
...
main().catch(async (err) => {
  console.error('[remapNaturalSciencesDomain] failed:', err);
  try {
    await mongoose.disconnect();
  } catch (_) {
    /* ignore */
